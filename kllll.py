...
async def handle_login(event):
    ...
    args = event.raw_text.split()
    if len(args) < 2:
        await event.reply("Format: /login session_name")
        return
    ...
    try:
        ...
        # This will send the sign-in code to the admin instead of terminal
        await client.start(phone=lambda: account["phone"], code_callback=lambda: asyncio.create_task(request_code_from_admin(event)))
        await event.reply(f"Successfully logged in to {session_name}!")
